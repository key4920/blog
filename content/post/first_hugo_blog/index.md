...
> **사전지식** : git도 commit add push 외에 안써봄, 웹 지식 없음, 전공자 아님  
...
- 이때 hugo -t stack 부분에서 'stack'을 본인 테마 이름으로 수정 해 준다.
...
원래는 이 파일 안에 한줄 한줄을 주옥같이 터미널에서 매번 실행 해 주어야하지만, 귀찮으므로 이런 파일을 만들고 파일만 실행하면 안의 코드가 실행되게 하는 원리이다.  
